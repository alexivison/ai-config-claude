...
run_hook "$(bash_input_obj 'party-cli transport review-complete /tmp/f.toon' "$COMBINED_STDOUT")"
...
clean_evidence
run_hook "$(bash_input_str 'party-cli transport review-complete /tmp/f.toon' "$COMBINED_STDOUT")"
assert "String APPROVED → codex evidence created" 'has_evidence "codex"'

echo "=== review-complete: APPROVED via chained command ==="
clean_evidence
run_hook "$(bash_input_obj 'cd /tmp && party-cli transport review-complete /tmp/f.toon' "$COMBINED_STDOUT")"
assert "Chained command APPROVED → codex evidence created" 'has_evidence "codex"'
...
run_hook "$(bash_input_obj 'party-cli transport review-complete /tmp/f.toon' "$RC_STDOUT")"
...
clean_evidence
run_hook "$(bash_input_obj 'party-cli transport review-complete /tmp/f.toon' 'CODEX APPROVED')"
...
clean_evidence
run_hook "$(bash_input_obj 'party-cli transport review-complete /tmp/f.toon' 'CODEX_REVIEW_RAN')"
...
clean_evidence
run_hook "$(bash_input_obj 'party-cli transport plan-review PLAN.md /tmp/work' 'CODEX_PLAN_REVIEW_REQUESTED')"
...
INPUT=$(jq -cn \
  --arg sid "$SESSION" \
  --arg cwd "$TMPDIR_BASE" \
  '{tool_name:"Bash",tool_input:{command:"party-cli transport review-complete /tmp/f.toon"},tool_response:{stdout:"CODEX_REVIEW_RAN\nCODEX APPROVED",stderr:""},tool_exit_code:1,session_id:$sid,cwd:$cwd}')
...
clean_evidence
run_hook "$(bash_input_obj 'party-cli transport review-complete /tmp/f.toon' 'Error' 1)" || true
assert "tool_response.exit_code=1 → no evidence" '! has_evidence "codex"'

# ═══ Guard clauses ═══════════════════════════════════════════════════════════

echo "=== Guard: Non-transport command ignored ==="
clean_evidence
run_hook "$(bash_input_obj 'echo CODEX_REVIEW_RAN' "$COMBINED_STDOUT")"
assert "Non-transport → no evidence" '! has_evidence "codex"'
...
run_hook "$(bash_input_obj 'party-cli transport review-complete /tmp/f.toon' "$APPROVED_STDOUT")"
...
run_hook "$(bash_input_obj 'party-cli transport triage-override code-critic "Out-of-scope: rebased auth files"' "$OVERRIDE_STDOUT")"
...
run_hook "$(bash_input_obj 'party-cli transport triage-override code-critic "No critic ran"' "$OVERRIDE_STDOUT2")"
...
run_hook "$(bash_input_obj 'party-cli transport triage-override codex "Trying to bypass"' "$OVERRIDE_BAD")"
...
run_hook "$(bash_input_obj 'party-cli transport review-complete /tmp/f.toon' "$COMBINED_OVERRIDE")"
